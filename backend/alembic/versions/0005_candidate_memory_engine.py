...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('id', sa.String(length=36), nullable=False),
        sa.Column('candidate_id', sa.String(length=36), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('interview_id', sa.String(length=36), sa.ForeignKey('interviews.id', ondelete='SET NULL'), nullable=True),
    ...
    op.create_index('ix_learning_recommendations_candidate_id', 'learning_recommendations', ['candidate_id'])
