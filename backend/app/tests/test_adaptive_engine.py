...
def test_adaptive_difficulty_escalation(db_session: Session):
    ...
    res1 = engine.process_answer_and_adjust(
        session.id, performance_score=85.0, response_latency_seconds=10.0
    )
    assert res1["new_difficulty"] > 5.0

    res2 = engine.process_answer_and_adjust(
        session.id, performance_score=92.0, response_latency_seconds=12.0
    )
...
def test_adaptive_difficulty_deescalation(db_session: Session):
    ...
    res = engine.process_answer_and_adjust(
        session.id, performance_score=45.0, response_latency_seconds=50.0
    )
