...
def test_1_two_month_candidate_has_basic_ceiling():
    ceiling = QuestionDifficultyPolicy.get_max_allowed_difficulty(relevant_experience_months=2, seniority_level="JUNIOR")
    assert ceiling == "BASIC"


def test_2_two_month_candidate_cannot_receive_hard():
    is_valid, reason = QuestionDifficultyPolicy.validate_question_difficulty("HARD", relevant_experience_months=2, seniority_level="JUNIOR")
    assert is_valid is False
    assert "exceeds candidate ceiling" in reason


def test_3_two_month_candidate_cannot_receive_advanced():
    is_valid, reason = QuestionDifficultyPolicy.validate_question_difficulty("ADVANCED", relevant_experience_months=2, seniority_level="JUNIOR")
    assert is_valid is False
    assert "exceeds candidate ceiling" in reason
...
def test_5_six_month_candidate_has_intermediate_ceiling():
    ceiling = QuestionDifficultyPolicy.get_max_allowed_difficulty(relevant_experience_months=6, seniority_level="JUNIOR")
    assert ceiling == "INTERMEDIATE"


def test_6_twenty_four_month_candidate_has_intermediate_ceiling():
    ceiling = QuestionDifficultyPolicy.get_max_allowed_difficulty(relevant_experience_months=24, seniority_level="MID")
    assert ceiling == "INTERMEDIATE"


def test_7_forty_eight_month_candidate_has_advanced_ceiling():
    ceiling = QuestionDifficultyPolicy.get_max_allowed_difficulty(relevant_experience_months=48, seniority_level="MID")
    assert ceiling == "ADVANCED"


def test_8_sixty_plus_month_senior_has_system_design_ceiling():
    ceiling = QuestionDifficultyPolicy.get_max_allowed_difficulty(relevant_experience_months=72, seniority_level="SENIOR")
...
def test_9_senior_title_with_two_months_experience_remains_basic():
    ...
    ceiling = QuestionDifficultyPolicy.get_max_allowed_difficulty(relevant_experience_months=2, seniority_level="INTERN")
    assert ceiling == "BASIC"
...
def test_14_valid_answer_proceeds_to_llm():
    res = AnswerSanityGuard.evaluate("FastAPI uses Pydantic models to validate request payloads asynchronously.")
...
def test_18_five_run_determinism_assertion():
    results = [
        QuestionDifficultyPolicy.get_max_allowed_difficulty(relevant_experience_months=2, seniority_level="JUNIOR")
        for _ in range(5)
    ]
    first = results[0]
    for r in results[1:]:
