...
class CompanyProfileEngine:
    ...
    def _seed_default_companies(self) -> None:
        defaults = [
            ("Amazon", "Amazon leadership principles & system scaling emphasis", 0.35, 0.25, 0.40, ["Customer Obsession", "Ownership", "Dive Deep", "Deliver Results"]),
            ("Google", "Google data structures, algorithms & system architecture", 0.50, 0.30, 0.20, ["Googliness", "Algorithm Complexity", "System Design"]),
            ("Microsoft", "Microsoft software engineering design & problem solving", 0.30, 0.40, 0.30, ["Growth Mindset", "Collaborative Design", "Quality"]),
            ("Meta", "Meta fast-paced coding, system design & product orientation", 0.45, 0.35, 0.20, ["Move Fast", "Focus on Impact", "Build Awesome Things"]),
        ]
        for name, desc, c_w, s_w, b_w, princ in defaults:
            existing = self.db.query(CompanyProfile).filter(CompanyProfile.company_name == name).first()
            if not existing:
                cp = CompanyProfile(
                    company_name=name,
                    description=desc,
    ...
    def get_company_profile(self, company_name: str) -> CompanyProfileResponse:
        comp = self.db.query(CompanyProfile).filter(CompanyProfile.company_name.ilike(company_name)).first()
        if not comp:
            comp = self.db.query(CompanyProfile).filter(CompanyProfile.company_name == "Amazon").first()

        principles = json.loads(comp.key_principles) if comp and comp.key_principles else ["Leadership", "Coding"]
