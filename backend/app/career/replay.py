        ...
        if not annotations:
            ...
            defaults = [
                (
                    "00:35",
                    "EXCELLENT",
                    "Clear, confident intro explaining microservice architecture",
                ),
                (
                    "02:11",
                    "WEAKNESS",
                    "Confidence dropped when questioned on database replication lag",
                ),
                (
                    "05:40",
                    "PAUSE",
                    "Long pause (12s) before answering rate limiting algorithm question",
                ),
                (
                    "09:10",
                    "MISSED_EDGE_CASE",
                    "Forgot to specify fallback cache strategy when Redis cluster is down",
                ),
                (
                    "13:05",
                    "EXCELLENT",
                    "Excellent optimization suggestion using B-Tree indexing and PgBouncer",
                ),
            ]
            items = []
            for ts, ann_type, note in defaults:
                ...
                self.db.add(ann)
                items.append(
                    InterviewAnnotationItem(timestamp_mark=ts, annotation_type=ann_type, note=note)
                )
            self.db.commit()
