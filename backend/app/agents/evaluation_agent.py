...
class EvaluationOutput(BaseModel):
    ...
    def sub_scores_in_range(self) -> EvaluationOutput:
        for dim, val in self.rubric_breakdown.items():
            if not (1 <= val <= 5):
                raise ValueError(
                    f"Sub-score for '{dim}' is {val} — must be 1–5."
                )
        return self
...
class EvaluationAgent(BaseAgent):
    ...
    def _run(self, state: InterviewState, retry_feedback: str | None = None) -> dict:
        ...
        competency    = current_q.get("competency_targeted") or latest_answer.get("competency_targeted") or "General"
        q_type        = current_q.get("question_type") or latest_answer.get("question_type") or "fundamentals"
        ...
        if round_type == "APTITUDE":
            ...
            q_text_clean = re.sub(r"[^\w\s]", "", (current_q.get("question_text") or "").lower()).strip()
        ...
        jd_skills_str = ", ".join(
            (jd_data.get("required_skills") or [])[:8]
        )
        resume_skills_str = ", ".join(
            (resume_data.get("skills") or [])[:8]
        )
        ...
        weight = next(
            (c.get("weight", 10) for c in matrix if c.get("name") == competency), 10
        )
